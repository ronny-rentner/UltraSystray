...
# Decorator for callback functions that should be used by Windows events
TypeConsoleCtrlHandler = ctypes.WINFUNCTYPE(wintypes.BOOL, wintypes.DWORD)

SetConsoleCtrlHandler = windll.kernel32.SetConsoleCtrlHandler
SetConsoleCtrlHandler.argtypes = (TypeConsoleCtrlHandler, wintypes.BOOL)
SetConsoleCtrlHandler.restype = wintypes.BOOL
CTRL_C_EVENT = 0

TypeMessageHandler = ctypes.WINFUNCTYPE(ctypes.HRESULT, wintypes.HWND, wintypes.UINT, wintypes.WPARAM, wintypes.LPARAM)
...
class MENUINFO(ctypes.Structure):
    _fields_ = [
        ...
        ('dwMenuData',       wintypes.LPVOID),
    ]
LPMENUINFO = ctypes.POINTER(MENUINFO)
...
class WNDCLASSEX(ctypes.Structure):
    _fields_ = [
        ('cbSize', wintypes.UINT),
        ('style', wintypes.UINT),
        ('lpfnWndProc', TypeMessageHandler),
...
SetMenuInfo.argtypes = (wintypes.HMENU, LPMENUINFO)
